...
export default class Statistics extends SmartView {
  ...
  _renderMoneyChart(moneyCtx, events) {
    ...
    getOptions(moneyCtx, TextChart.MONEY, getTypesUniq(events), totalPriceByTypes, (val) => `€ ${val}`);
  }
  ...
  _renderTypeChart(typeCtx, events) {
    ...
    getOptions(typeCtx, TextChart.TYPE, getTypesUniq(events), countByEventTypes, (val) => `${val}x`);
  }
  ...
  _renderTimeChart(timeCtx, events) {
    ...
    getOptions(timeCtx, TextChart.TIME, getTypesUniq(events), durationEventTypes, (val) => `${humanizeDuration(val)}`);
  }
